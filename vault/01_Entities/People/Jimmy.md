---
type: entity-person
tags: [entity, person, high]
role: President
organization: "[[Asphalt Transport Inc (ATI)]]"
risk_level: high
status: confirmed
created: 2026-03-23
updated: 2026-03-24
related: ["[[Asphalt Transport Inc (ATI)]]", "[[Rosanna Karim]]"]
known-accounts: ["ATI corporate (President)"]
---

# Jimmy (ATI President)

## Role & Access
- **Title:** President
- **Organization:** [[Asphalt Transport Inc (ATI)]]
- **Authority:** Executive authority over ATI operations

## Relevance to Investigation
As ATI President, Jimmy has ultimate authority over the entity controlling Apollo's finances. Whether directly involved in or aware of the alleged misappropriation is a key question.

## Strategic Notes
Strong ego — will resist on pride even when merits are against him. Communications must be crafted carefully to avoid adversarial framing that triggers defensive responses.

## QB Log Statistics (from [[DOC-QB-LOGS-2022-2026]])
- **Total entries:** 10 (<0.1% — minimal direct system involvement)
- **General Journal entries:** 0
- Limited direct fingerprint in the system, but executive authority over all ATI actions

## Known Accounts
- ATI corporate accounts (President authority)
- Oversight of all Apollo/Gemini financial operations via ATI control

## Key Transactions
| Date | Amount | Type | Source |
|------|--------|------|--------|
| Various 2024 | $335,566 | Total ATI extraction from Apollo (authorized) | [[Intercompany Transfer Tracker]] |
| Various 2025 | $866,092 | Total ATI extraction from Apollo (authorized) | [[Intercompany Transfer Tracker]] |

## Connected Findings
- [[Fuel Cost Manipulation]] — ATI-level mechanism
- [[ATI Monthly Billing Manipulation]] — ATI-level billing
- [[Promissory Note Backdating]] — ATI-level note
- [[GL Audit Trail Anomalies]] — No audit trail on system he oversees
- [[Gemini Record Manipulation]] — Related entity under his authority
- [[Intercompany Transfer Tracker]] — $1.2M gross extraction from Apollo
- [[Master Timeline]]
